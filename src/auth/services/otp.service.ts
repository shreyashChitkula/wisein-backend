...
export class OtpService {
  ...
  async sendOtp(
    email: string,
    options?: { username?: string; purpose?: 'register' | 'login' },
  ): Promise<{ message: string; status: number; userId?: string }> {
    const purpose =
      options?.purpose ?? (options?.username ? 'register' : 'login');
    this.logger.log(
      `[sendOtp] Processing send-otp for ${email} purpose=${purpose} username=${options?.username}`,
    );
    ...
    if (purpose === 'register') {
      ...
      const user = await this.prisma.user.create({
        data: { email, username, role: 'INDIVIDUAL', status: 'REGISTERED' },
      });
      ...
      this.logger.debug(
        `[sendOtp] OTP stored for ${email} (register), expires at ${expiresAt}`,
      );
      ...
      try {
        ...
        this.logger.warn(
          `[sendOtp] Email failed for ${email}, console fallback: ${err?.message}`,
        );
    ...
    if (purpose === 'login') {
      ...
      if (!user) {
        this.logger.warn(
          `[sendOtp] Login requested for unknown user: ${email}`,
        );
      ...
      if (user.status === 'REGISTERED') {
        this.logger.warn(
          `[sendOtp] Login requested for unverified user: ${email}`,
        );
      ...
      this.logger.debug(
        `[sendOtp] OTP stored for ${email} (login), expires at ${expiresAt}`,
      );
  ...
  async verifyOtp(
    email: string,
    otpCode: string,
  ): Promise<{ userId: string; purpose: 'register' | 'login' }> {
    ...
    if (otpRecord.attempts >= 3) {
      this.logger.warn(
        `[verifyOtp] Too many failed attempts for email ${email}`,
      );
      this.otpStore.delete(email);
    ...
    if (otpRecord.otp !== otpCode) {
      this.logger.debug(
        `[verifyOtp] Invalid OTP attempt for email ${email}, attempts: ${otpRecord.attempts + 1}`,
      );
    ...
    this.logger.log(
      `[verifyOtp] OTP verified successfully for ${email} purpose=${otpRecord.purpose}`,
    );
    ...
    if (otpRecord.purpose === 'register') {
      ...
      if (!user) {
        this.logger.error(
          `[verifyOtp] User not found after successful OTP: ${email}`,
        );
